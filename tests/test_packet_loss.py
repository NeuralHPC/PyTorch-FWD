...
def test_packet_loss_unit_weights():
    ...
    assert np.allclose(packet_loss.item(), 0.0)
...
def test_mixed_loss_all_combinations():
    ...
    assert np.allclose(mixed_loss_1.item(), 0.0)
    assert np.allclose(mixed_loss_2.item(), 0.0)
    assert np.allclose(mixed_loss_3.item(), 0.0)
