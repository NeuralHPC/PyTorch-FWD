...
if __name__ == "__main__":
    ...
    net_weights = torch.load(
        "../weights/inception_converged_celeba/inception_converged_99.pth"
    )
    ...
    original_loader = DataLoader(
        SimpleLoader("../data/agri/dataset256/images/"), batch_size=128, num_workers=6
    )
    ddgan_loader = DataLoader(
        SimpleLoader("../data/agri/generated_samples/agri"),
        batch_size=128,
        num_workers=6,
    )
    pggan_loader = DataLoader(
        SimpleLoader("../data/agri/out", type="png"), batch_size=128, num_workers=6
    )
    ...
    fid_agri_ddgan = pytorch_fid.fid_score.calculate_frechet_distance(
        agri_mu, agri_sigma, ddgan_mu, ddgan_sigma
    )
    fid_agri_pggan = pytorch_fid.fid_score.calculate_frechet_distance(
        agri_mu, agri_sigma, pggan_mu, pggan_sigma
    )
    ...
    # celebA-HQ
    original_loader = DataLoader(
        SimpleLoader("../data/celeba_hq/celeba_hq_256"), batch_size=128, num_workers=6
    )
    ddgan_loader = DataLoader(
        SimpleLoader("../data/celeba_hq/DDGAN/celeba_256"),
        batch_size=128,
        num_workers=6,
    )
    pggan_loader = DataLoader(
        SimpleLoader("../data/celeba_hq/pggan_celebahq_generated", type="png"),
        batch_size=128,
        num_workers=6,
    )
    ...
    fid_celebA_ddgan = pytorch_fid.fid_score.calculate_frechet_distance(
        celebA_mu, celebA_sigma, ddgan_mu, ddgan_sigma
    )
    fid_celebA_pggan = pytorch_fid.fid_score.calculate_frechet_distance(
        celebA_mu, celebA_sigma, pggan_mu, pggan_sigma
    )
