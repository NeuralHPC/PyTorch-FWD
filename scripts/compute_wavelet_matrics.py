...
original_loader = torch.utils.data.DataLoader(
    ImageDataset(original_path), batch_size=1, num_workers=11, prefetch_factor=11
)
sample_loader = torch.utils.data.DataLoader(
    ImageDataset(sample_path), batch_size=1, num_workers=11, prefetch_factor=11
)
...
for idx, arg in enumerate(tqdm(min_idx)):
    ...
    plt.axis("off")
    plt.suptitle(
        f"{round(wv_score[idx, arg], 2)},{round(fwd.item(), 2)}, \n {ref_files[idx]}, \n {sample_files[arg]}",
        fontsize=6,
    )
    plt.savefig(
        f"./WPSKL_matched_imgs_StyleGAN2/img_{idx}.png", bbox_inches="tight", dpi=600
    )
    plt.close()
