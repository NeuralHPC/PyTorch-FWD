...
from src.freq_math import forward_wavelet_packet_transform, wavelet_packet_power_divergence
import torch
from random import shuffle
from torchvision import transforms


original_image_path = '/home/lveerama/results/metrics_sampled_images/curated_original/*.jpg'
...
dataloader = torch.utils.data.DataLoader(ImageDataset(sampled_image_path), batch_size=1, num_workers=11, prefetch_factor=11)
...
for idx, arg in enumerate(min_idx):
    ...
    plt.imshow(sample_img)
    plt.axis('off')
    plt.suptitle(f'{round(fid_matrix[idx, arg], 2)},{round(fwd.item(), 2)}, \n {ref_files[idx]}, \n {sample_files[arg]}', fontsize=6)
