...
def sample_noise(
    img: jnp.ndarray, current_time_step: int, key: jnp.ndarray, max_steps: int
) -> Tuple[jnp.ndarray]:
...
def sample_DDPM(
    ...
    """
    prng_key = jax.random.PRNGKey(key)
    x_t = jax.random.normal(prng_key, shape=[1] + input_shape)
    x_t_1 = x_t
    steps = [x_t_1]
    ...
    for time in tqdm(reversed(range(max_steps)), total=max_steps):
        ...
        z = jax.random.normal(prng_key, shape=[1] + input_shape)
        denoise = model.apply(
            net_state,
            (
                x_t_1,
        ...
        x_mean = (x_t_1 - (denoise * ((1 - alpha) / (jnp.sqrt(1 - alpha_t))))) / (
            jnp.sqrt(alpha)
        )
...
def batch_DDPM(
    ...
    """
    prng_key = jax.random.PRNGKey(key)
    x_t = jax.random.normal(prng_key, shape=[batch_size] + input_shape)
    ...
    def fori_image(i, carry):
        ...
        _, prng_key = jax.random.split(prng_key)
        z = jax.random.normal(prng_key, shape=[batch_size] + input_shape)
        denoise = model.apply(
            net_state, (x_t_1, jnp.array([time] * batch_size), test_label)
        )
        x_mean = (x_t_1 - (denoise * ((1 - alpha) / (jnp.sqrt(1 - alpha_t))))) / (
            jnp.sqrt(alpha)
        )
...
def sample_DDIM(
    ...
    for time in tqdm(
        reversed(range(0, max_steps, tau_steps)), total=(max_steps // tau_steps) + 1
    ):
        ...
        if time == 0:
            alpha_t_1 = 1.0
        sigma_t = eta * (
            (jnp.sqrt((1 - alpha_t_1) / (1 - alpha_t)))
            * (jnp.sqrt((1 - alpha_t) / (alpha_t_1)))
        )
        _, prng_key = jax.random.split(prng_key)
        z = jax.random.normal(prng_key, shape=[1] + input_shape)
        denoise = model.apply(
        ...
        pred_x_0 = jnp.sqrt(alpha_t_1 / alpha_t) * (
            (x_t_1 - (jnp.sqrt(1 - alpha_t) * denoise))
        )
