...
from src.sample_util import sample_DDPM, sample_DDIM
...
class Sampler:
    def __init__(self,
        ...
        if len(std) != 0 and len(mean) != 0:
            self.__std = torch.reshape(torch.tensor(std), (1, 3, 1, 1)).to(self.__local_rank)
            self.__mean = torch.reshape(torch.tensor(mean), (1, 3, 1, 1)).to(self.__local_rank)
